...
/* CONTENIDO DIV SECUNDARIO  ******************************************* */
            ...
            for (var i = 0; i < 4; i++) {
                var habilidadIndex = i;  // Usa una variable diferente para el contador del bucle
                var nuevaHabilidad = document.createElement("h2");
                nuevaHabilidad.classList.add("habilidad");
                nuevaHabilidad.textContent = data.data[i].abilities[habilidadIndex].displayName;
                divpersonajesecundario.appendChild(nuevaHabilidad);
                console.log(data.data[i].abilities[habilidadIndex].displayName);
            }
